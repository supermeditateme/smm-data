...
import { createAnnotation, selectAnnotation } from '../../actions/annotations.js';
import { getSelectedAnnotation } from '../../selectors/annotations.js';
import { getSecondWidth, getCurrentPosition } from '../../selectors/navigation.js';
  ...
  function CtrlClickCatcher(props) {
    ...
    _this2.onMouseUp = function (e) {
      ...
      _this2.setState({ ctrlPressed: e.ctrlKey, savedX: null, delta: null }, function () {
        ...
        _this2.props.dispatch(createAnnotation({
          id: id,
          name: nameForAnnotation,
          time: (currentPosition + startX) / secondWidth,
          duration: width / secondWidth
        }));
        _this2.props.dispatch(selectAnnotation(id));
      });
    };

    _this2.catcherRef = React.createRef();
  ...
  _createClass(CtrlClickCatcher, [{
    ...
    value: function render() {
      ...
      return React.createElement(
        ...
        React.createElement(NewSelection, { savedX: savedX, delta: delta })
      );
    }
  }]);
...
var mapStateToProps = function mapStateToProps(state) {
  return {
    ...
    nameForAnnotation: (getSelectedAnnotation(state) || state.annotations.list[0] || { name: 'New annotation' }).name,
    secondWidth: getSecondWidth(state),
    currentPosition: getCurrentPosition(state)
  };
};

var CtrlClickCatcher$1 = connect(mapStateToProps, undefined, undefined, {
  storeKey: STORE_KEY
})(CtrlClickCatcher);

export default CtrlClickCatcher$1;
